...
import sys
import re
from metrics import evaluate_file, get_file_encoding, write_lines
...
def read_eval_index_dataset(data_path, is_label=True):
    ...
    with open(data_path, "r", encoding=get_file_encoding(data_path)) as reader:
        while True:
...
def substitution_ranking(source_word, source_context, substitution_selection, fasttext_dico, fasttext_emb, word_count, ssPPDB, tokenizer, maskedLM, lables):
    ...
    candidates = []
    all_ranks2 = all_ranks.copy()
    all_ranks2 = sorted(all_ranks2)
    
    for rank in all_ranks2:
        pre_index = all_ranks.index(rank)
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for i in range(eval_size):
            ...
            candidates = substitution_ranking(mask_words[i], mask_context, cgBERT, fasttext_dico, fasttext_emb,word_count,cgPPDB,tokenizer,model,mask_labels[i])


            substitution_words.append(candidates)
            line = f'{eval_examples[i]}\t{complex_word}\t' + '\t'.join(candidates)
            output_lines.append(line)
            
        gold_filepath = Path(args.eval_dir)
        results_dir = Path('results_conls')
        results_dir.mkdir(parents=True, exist_ok=True)
        pred_filepath = results_dir / f'{gold_filepath.stem}.tsv'
        write_lines(output_lines, pred_filepath)
        # results_filepath = results_dir / f'{pred_filepath.stem}.csv'
        # evaluate_file(pred_filepath, gold_filepath, results_filepath)
